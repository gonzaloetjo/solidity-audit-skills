...
- **Automated eval harness** (`evals/scripts/run-eval.sh`): Shell script that runs the eval skill against fixtures, grades output, and extracts cost metadata. Supports `--fixture <name>` or `--all`, `--trials N` for multi-trial evaluation, `--max-budget-usd` for cost caps, `--model` for model selection (sonnet, opus, haiku, or full model ID), and `--naive` for raw-model baseline comparison (no pipeline). Handles fixture isolation (temp dir copy), forge-std installation, `claude -p` invocation, and grade.json generation per trial. Naive results stored separately under `results/naive-{model}/`.
