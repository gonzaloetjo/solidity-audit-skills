...
**Body structure**: The markdown body IS the agent prompt. Keep SKILL.md between 200-350 lines. Move detailed reference material (prompt templates, output examples, analysis templates) into `resources/` files that agents read at runtime.
...
**Stage pipeline** — Skills define 6 sequential stages: Stage 0 (design decisions, interactive), Stages 1-3 (foundation, domain audit, cross-cutting — agents in parallel), Stage 4 (human review, interactive), Stage 5 (re-evaluation, conditional agent). Later stages depend on earlier stage outputs.
...
Both variants run the same 6-stage pipeline (Stage 0 → 1 → 2 → 3 → 4 → 5). Stages 0, 4, 5 are orchestrator-interactive and identical across variants. Only Stages 1-3 differ (solo uses background agents, team uses agent teams).
