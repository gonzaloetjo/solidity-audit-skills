...
**Stage pipeline** — Skills define 6 sequential stages: Stage 0 (design decisions, interactive + optional Slither), Stages 1-3 (foundation, domain audit, cross-cutting — agents in parallel, plus companion skill agents in Stage 3), Stage 4 (human review, interactive), Stage 5 (re-evaluation, conditional agent). Later stages depend on earlier stage outputs.

**Slither integration** — If Slither is installed, the orchestrator runs it between Stage 0 and Stage 1. Findings go to `stage0/slither-findings.md`. All agents cross-reference with Slither via the `{slither_file}` placeholder.

**Companion skill agents** — The orchestrator detects installed companion skills (Trail of Bits' `trailofbits/skills` marketplace) during pre-flight. Relevant skills spawn additional Stage 3 agents that apply the companion skill's methodology. Output goes to `stage3/companion-{skill-slug}.md`.
...
Both variants run the same 6-stage pipeline (Stage 0 → Slither → 1 → 2 → 3 → 4 → 5). Stages 0, 4, 5 are orchestrator-interactive and identical across variants. Slither integration is identical. Only Stages 1-3 differ (solo uses background agents, team uses agent teams). Both variants detect and spawn companion skill agents in Stage 3.

FUNCTION_TEMPLATE.md, EXAMPLE_OUTPUT.md, and REVIEW_PROMPTS.md are byte-identical across both plugins. STAGE_PROMPTS.md differs only by Communication Guidelines sections in the team variant (including companion agent communication guidelines).
